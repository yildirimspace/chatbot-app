...
# --- Chat input + inline disclaimer attached to the chat bar ---
st.markdown(
    """
    <style>
    ...
    /* Disclaimer that moves with the chat bar */
    [data-testid="stChatInput"]::after {
        content: "Responses are generated by an AI model and may be inaccurate or incomplete. Always cross-check important information with the original report and other sources.";
        position: absolute;
        left: 50%;
        bottom: -3.6rem;          /* a bit further under the input than before */
        transform: translateX(-50%);
        font-size: 0.75rem;
        color: #888888;
        text-align: center;
        width: 95%;               /* keep it away from the very edges */
        max-width: 60rem;
        pointer-events: none;     /* don’t block clicks */
        white-space: normal;      /* allow wrapping cleanly */
    }
    </style>
    """,
